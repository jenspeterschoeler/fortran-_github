...
!        call system('ls' ) ! Linux
        ...
        do
            ...
            case('T', 't')
                ntb = ntb + 1
            case('SPH', 'sph' , 'SFH', 'sfh')
                nhp = nhp + 1
            case('SPC', 'spc', 'SFC', 'sfc')
                ncp = ncp + 1
            case('BFUNIF', 'bfunif')
                nq = nq + 1
            case('BFE', 'bfe')
		...
		if (nn == 0) then
            ...
            stop
        else if (nb == 0 .AND. ntb == 0 .AND. ncp == 0) then
          	write(*,*) 'ERROR: No supports or thermal bounds defined'
            stop
        else if (nb == 0) then
            ...
            write (*, *) 'WARNING: No nodal temperatures defined'
        else if (nhp == 0) then
            write (*, *) 'WARNING: No element surface heat flux defined'
        else if (ncp == 0) then
            write (*, *) 'WARNING: No element convection defined'
        else if (nq == 0) then
            write (*, *) 'WARNING: No heat generation defined'
        else if (nqe == 0) then
            write (*, *) 'WARNING: No element heat generation defined'
        end if
        ...
        allocate (tbound(ntb,3), convloads(ncp,5), fluxloads(nhp,4), qgen(nq,2)&
        ,qegen(nqe,2))
        ...
        do
            ...
            case ('MP', 'mp')
                ...
                case ('KXX', 'kxx')
                    mprop(ivalue)%cond = rvalue
                case ('C', 'c')
                    mprop(ivalue)%c_specific = rvalue
                case ('CTEX', 'ctex')
            ...
            case ('T', 't')
                ...
                case ('TEMP', 'temp')
                    ...
                    tbound(ntb, 3) = rvalue
                case default
                	write (*, *) 'ERROR: Nodal dof "T"'
                    stop
                end select
            case ('SPC', 'spc', 'SFC', 'sfc')
                backspace (10)
                ncp = ncp + 1
                read (10, *) command, ivalue, ivalue2, cvalue, cvalue2, rvalue, rvalue2
                convloads(ncp, 1) = 2
                convloads(ncp, 2) = ivalue
                convloads(ncp, 3) = ivalue2
                convloads(ncp, 4) = rvalue
                convloads(ncp, 5) = rvalue2
            case ('SPH', 'sph', 'SFH', 'sfh')      
            backspace (10)
                nhp = nhp + 1
                read (10, *) command, ivalue, ivalue2, cvalue, cvalue2, rvalue
                fluxloads(nhp, 1) = 3
                fluxloads(nhp, 2) = ivalue
                fluxloads(nhp, 3) = ivalue2
                fluxloads(nhp, 4) = rvalue
            case ('BFUNIF', 'bfunif')
            	backspace (10)
                nq = nq + 1
                read(10, *) command, cvalue, rvalue
                qgen(nq, 1) = 4
                qgen(nq, 2) = rvalue
            case ('BFE', 'bfe')
            	backspace (10)
        ...
        do
          ...
          case ('ANTYPE', 'antype')
                ...
                case ('THERMAL', 'thermal')
                    antype = 'thermal'
                case ('THERMSTAT', 'thermstat')
                    antype = 'thermstat'
                case default
                    write (*, *) 'ERROR: Only static, static_nl, modal, angle, trans, therml and'
                    write (*, *) 'thermalstatic analyses are implemented'
                    stop
                end select
            end select
        end do
...
!
    subroutine output
        ...
        write (10, *)
        write (10, '("Element                    Principal stresses             Von mises stress")')
        write (10, '("number     1st principal    2nd principal   Angle                   ")')          
        do e = 1, ne
            select case( element(e)%id )
            case( 1 )
                write (10, *)
            case( 2 )
                write (10, '(1x, i6, 3(1x, f15.9), 1x, 3(1x, f15.9))') e, (principal_stresses(e, i), i = 1, 3) ,(stress_vm(e))
            end select
        end do
        if (antype == 'thermstat') then
           write (10, '(" Node            Temperatures  ")')
           do i = 1, nn
            	write (10, '(i6, 1x, f15.9, 1x, f15.9, 2x, f15.9, 1x, f15.9)') i, t(i), d(2*i), p(2*i-1), p(2*i)
           end do
		end if
        
        write (10, *)
        write (10, '("Compliance        Max Von mises stress		Average Area of elements")')
        write(10, *) c, max_vm, h2        
...
!
    subroutine plot( what,                                &
        ...
        real :: clx, cly, mine, maxe, h, w, wsize, aspect, x0, y0, wemax, pemax, wxmin, wxmax, wymin, wymax
        ...
        if (iand(what, vectors) /= 0) then
            ...
            pemax = real(max( maxval(abs(p1)), maxval(abs(p2)) ))
        end if
        ...
        do e = 1, ne
			...
			if (antype == 'thermal') then
            	!print*,'No deformed geometry in thermal antype'
            ...
            if (iand(what, vectors) /= 0) then
                ...
                sf = wemax / pemax * scale_vec * 0.9_wp
                ...
                ca = cos(-ang(e))
                sa = sin(-ang(e))
                xbox(1) = real(avepx - 0.5*sf*abs(p1(e))*ca)
                ybox(1) = real(avepy - 0.5*sf*abs(p1(e))*sa)
                xbox(2) = real(avepx + 0.5*sf*abs(p1(e))*ca)
                ybox(2) = real(avepy + 0.5*sf*abs(p1(e))*sa)
                if (p1(e) >= 0) then
                    call pgsci(cu)
                else
                    call pgsci(cd)
                ...
                ca = cos(-ang(e)+pi/2)
                sa = sin(-ang(e)+pi/2)
                xbox(1) = real(avepx - 0.5*sf*abs(p2(e))*ca)
                ybox(1) = real(avepy - 0.5*sf*abs(p2(e))*sa)
                xbox(2) = real(avepx + 0.5*sf*abs(p2(e))*ca)
                ybox(2) = real(avepy + 0.5*sf*abs(p2(e))*sa)
                if (p2(e) >= 0) then
                    call pgsci(cu)
                else
                    call pgsci(cd)
