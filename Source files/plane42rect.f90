...
!--------------------------------------------------------------------------------------------------
!
    subroutine plane42rect_ss(xe, de, young, nu, estress, estrain, principal_stresses_e, sigma_vm, h, w)
        ...
        bmat = 0
        bmat_fact = 1/(4*aa*bb)
		bmat(1,1) = -(bb-y)*bmat_fact
        ! bmat(2,1) = 0
        bmat(3,1) = -(aa-x)*bmat_fact
        ! bmat(1,2) = 0
		bmat(2,2) = -(aa-x)*bmat_fact
        bmat(3,2) = -(bb-y)*bmat_fact
        bmat(1,3) = (bb-y)*bmat_fact
		! bmat(2,3) = 0
        bmat(3,3) = -(aa+x)*bmat_fact
		! bmat(1,4) = 0
		bmat(2,4) = -(aa+x)*bmat_fact
        bmat(3,4) = (bb-y)*bmat_fact
        bmat(1,5) = (bb+y)*bmat_fact
		! bmat(2,5) = 0
        bmat(3,5) = (aa+x)*bmat_fact
        ! bmat(1,6) = 0
		bmat(2,6) = (aa+x)*bmat_fact
        bmat(3,6) = (bb+y)*bmat_fact
		bmat(1,7) = -(bb+y)*bmat_fact
		! bmat(2,7) = 0
        bmat(3,7) = (aa-x)*bmat_fact
		! bmat(1,8) = 0
		bmat(2,8) = (aa-x)*bmat_fact
        bmat(3,8) = -(bb+y)*bmat_fact
        ...
        sigma_1 = 0.5*(estress(1)+estress(2))+sqrt((0.5*(estress(1)+estress(2)))**2+estress(3)**2)
        sigma_2 = 0.5*(estress(1)+estress(2))-sqrt((0.5*(estress(1)+estress(2)))**2+estress(3)**2)
        sigma_vm = sqrt(sigma_1**2+sigma_2**2-sigma_1*sigma_2)
        psi = atan2(-2*estress(3)/(sigma_1-sigma_2),(estress(1)-estress(2))/(sigma_1-sigma_2))*0.5
